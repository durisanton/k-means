...
class GUI:
    def __init__(self,gui):
        ...
        #Buttons
        self.random_centroid_coordinates_2d = Button(self.gui,text = 'Random centroid coordinates 2D', command = self.random_centroids)
        self.random_centroid_coordinates_2d.grid(row = 9, column = 3, columnspan=2)
        self.random_centroid_coordinates_3d = Button(self.gui,text = 'Random centroid coordinates 3D', command = self.random_centroids_3d)
    ...
    def browse(self):
        filename =  filedialog.askopenfilename(title = "Select file",filetypes = (("csv files","*.csv"),("txt files","*.txt*")))
    ...
    def random_centroids(self):
        ...
        self.centroids = {i+1: [round(np.random.uniform(self.data['x'].min(), self.data['x'].max()),2), round(np.random.uniform(self.data['y'].min(), self.data['y'].max()),2)] for i in range(int(self.cluster_quantity.get()))}
        self.listbox.delete('1.0', END)
        for i in self.centroids.keys():
    ...
    def random_centroids_3d(self):
        self.state = '3d'
        # Making new centroid coordinates
        self.centroids = {i+1: [round(np.random.uniform(self.data['x'].min(), self.data['x'].max()),2), round(np.random.uniform(self.data['y'].min(), self.data['y'].max()),2), round(np.random.uniform(self.data['z'].min(), self.data['z'].max()),2)] for i in range(int(self.cluster_quantity.get()))}
        self.listbox.delete('1.0', END)
        for i in self.centroids.keys():
            self.listbox.insert(END, '[' + str(self.centroids[i][0]) + ',' + str(self.centroids[i][1]) + ',' + str(self.centroids[i][2])  + ']\n')
        #show random starting centroids
        self.fig.clf()
        self.ax = self.fig.gca(projection='3d')
        self.ax.scatter(self.data['x'], self.data['y'], self.data['z'], alpha=0.5, edgecolor='k')
    ...
    def metric_selection(self):
        ...
        if metric == 'Euclidean':
            ...
            for i in self.centroids.keys():
                if self.state == '2d':
                    self.data['distance_from_{}'.format(i)] = (np.sqrt((self.data['x'] - self.centroids[i][0]) ** 2 + (self.data['y'] - self.centroids[i][1]) ** 2))
                else:
                    self.data['distance_from_{}'.format(i)] = (np.sqrt((self.data['x'] - self.centroids[i][0]) ** 2 + (self.data['y'] - self.centroids[i][1]) ** 2) + (self.data['z'] - self.centroids[i][2]) ** 2)
        else:
            # Manhattan Distance between points
            for i in self.centroids.keys():
                if self.state == '2d':
                    self.data['distance_from_{}'.format(i)] = ((abs(self.data['x'] - self.centroids[i][0])) + (abs(self.data['y'] - self.centroids[i][1])))
                else:
                    self.data['distance_from_{}'.format(i)] = ((abs(self.data['x'] - self.centroids[i][0])) + (abs(self.data['y'] - self.centroids[i][1])) + (abs(self.data['z'] - self.centroids[i][2])))
    ...
    def show(self):
        if self.state == '2d':
            ...
            self.fig.clf()
            self.ax = self.fig.gca(projection='3d')
            self.ax.scatter(self.data['x'], self.data['y'], self.data['z'], color=self.data['color'], alpha=0.5, edgecolor='k')
            for i in self.centroids.keys():
                self.ax.scatter(*self.centroids[i], marker='x', color=self.colmap[i])
            self.ax.set_xlabel('X')
            self.ax.set_ylabel('Y')
    ...
    def previous_step(self):
        if ('closest' in self.data.keys()):     
            if self.state == '2d':
                ...
                self.ax = self.fig.gca()
                self.ax.grid()
                self.ax.scatter(self.old_data['x'], self.old_data['y'], color=self.old_data['color'], alpha=0.5, edgecolor='k')
                ...
                self.fig.clf()
                self.ax = self.fig.gca(projection='3d')
                self.ax.scatter(self.old_data['x'], self.old_data['y'], self.old_data['z'], color=self.old_data['color'], alpha=0.5, edgecolor='k')
                for i in self.old_centroids.keys():
                    self.ax.scatter(*self.old_centroids[i], marker = 'x',color=self.colmap[i])
                self.ax.set_xlabel('X')
                self.ax.set_ylabel('Y')
